...
| Key | Content |
|---|---|
| `GEOKLEIN_1_TOOL` | `GeoKlein Raster Optimiser {version}, a QGIS plugin, {D Month YYYY}. Source: {GitHub URL}. Listing: {plugins.qgis.org URL}` - version read from `metadata.txt`, never hardcoded; date is the day the conversion ran. Two labelled URLs since 1.0.1: the GitHub repository (`https://github.com/GeoKlein-Ltd/raster-optimiser`), the plugin's canonical home, and the plugins.qgis.org listing (`https://plugins.qgis.org/plugins/raster_optimiser/`), where a reader checks for or installs updates from inside QGIS. Labelled rather than left as two bare adjacent URLs, which would look structurally identical and not say which is which. |
...
> GEOKLEIN_1_TOOL = GeoKlein Raster Optimiser 1.0.0, a QGIS plugin, 26 August 2026. Source: https://github.com/GeoKlein-Ltd/raster-optimiser. Listing: https://plugins.qgis.org/plugins/raster_optimiser/
