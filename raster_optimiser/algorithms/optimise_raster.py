...
class OptimiseRasterAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        ...
        return self.tr(
            ...
            "unchanged.</p>"
            "<p>Not every file can be written for <i>Viewing</i>. "
            "Elevation, 16-bit and multispectral imagery are always "
            "written for <i>Analysis</i>. So are 8-bit RGB files whose "
            "transparent border is marked by a NoData value rather than "
            "an alpha band, because <i>Viewing</i> shifts pixel values "
            "slightly and a border marked by value would no longer "
            "match. A border marked by an alpha band is defined by "
            "position, so it survives. Where <i>Analysis</i> is forced, "
            "the tool says why in the log and in the file's own "
            "metadata, under Layer Properties, Information.</p>"
