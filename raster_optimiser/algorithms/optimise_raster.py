...
class OptimiseRasterAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        ...
        return self.tr(
            "<p>What does it do?</p>"
            "<p>Makes large rasters load and pan quickly in QGIS, "
            "QField, or any other GDAL-based software, and reduces "
            "their file size. A raster does not have to be large to be "
            "slow. The output is always a Cloud Optimized GeoTIFF "
            "(COG), never a .jpg file.</p>"

            "<p>Reasons your file is slow</p>"
            "<p>Most orthomosaics and elevation rasters come out of "
            "processing software without two things GDAL needs to draw "
            "them quickly.</p>"
            "<ul>"
            "<li>No pyramids. QGIS reads every pixel in the file to "
            "draw a zoomed-out view, and does it again on every pan and "
            "every zoom.</li>"
            "<li>No tiling. QGIS cannot read one corner of the image "
            "without reading the full width of every row that corner "
            "sits in.</li>"
            "</ul>"
            "<p>Both mean far more work than the view on screen needs. "
            "This tool adds pyramids and tiling, and picks a "
            "compression setting appropriate for what the raster "
            "contains, so it also takes up less disk space.</p>"

            "<p>What does each parameter do?</p>"
            "<p><i>Input layer</i>: the raster to optimise. Anything "
            "GDAL can read, apart from classified rasters and files "
            "with no coordinate reference system.</p>"
            ...
            "considerably smaller. A typical drone orthomosaic can come "
            "out around 80% smaller. Use it for basemaps, client "
            ...
            "choice affects file size, and whether pixel values survive "
            "unchanged.</p>"
            "<p>Not every file can be written for Viewing. Elevation, "
            "16-bit and multispectral imagery are always written for "
            "Analysis. So are some 8-bit RGB files, depending on how "
            "their transparency is stored. Where that applies, the "
            "tool writes for Analysis and says why in the log and in "
            "the file's own metadata, under Layer Properties, "
            "Information.</p>"
            "<p>If you are not sure, choose Analysis. It costs disk "
            "space and nothing else.</p>"
            "<p><i>Optimised raster</i>: where to save the result. "
            "Your source file is never modified.</p>"
            "<p>The three settings below are under Advanced "
            "parameters.</p>"
            ...
            "for an empty one.</li>"
            "<li>Band: one layer of values in a raster. A colour "
            ...
            "<li>Bit depth: how much range each pixel value has. "
            "8-bit holds 0 to 255, which is enough for colour. 16-bit "
            "and Float32 hold far more, which is what measurements "
            "need.</li>"
            "<li>Cloud Optimized GeoTIFF (COG): a GeoTIFF that is "
            "tiled, has pyramids, and keeps its index at the front of "
            "the file rather than after the image data. Software can "
            ...
            "area, where the image does not fill the rectangular file. "
            "Keeping it transparent is the alpha band's job. Without "
            "one you get a black box around your raster.</li>"
            "<li>Lossless and lossy: lossless compression makes a "
            "file smaller with every pixel value still recoverable "
            ...
            "can pick a value no real height could be, such as -9999. "
            "Risky on 8-bit imagery, where every value from 0 to 255 "
            "is a legitimate colour and 0 means black.</li>"
            "<li>Pyramids, also called overviews: pre-built smaller "
            "copies of the whole image at successive zoom levels. "
            "Faster because QGIS loads one small copy instead of the "
            "full image.</li>"
            "<li>Resampling: working out the pixel values for a "
            "smaller copy of an image from the pixels it replaces. "
            "Averaging is the usual method, and it is why pyramids "
            "cannot be built safely on classified rasters.</li>"
    ...
    def initAlgorithm(self, config=None):
        ...
        purpose_param.setHelp(self.tr(
            ...
            "analysis, and some 8-bit RGB files can too, depending on "
            "how their transparency is stored. Where that applies the "
            "tool writes for analysis instead, and explains why in the "
            "log and in the file itself.\n"
            "\n"
