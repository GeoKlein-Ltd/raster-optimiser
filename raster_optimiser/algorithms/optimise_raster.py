...
class OptimiseRasterAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        ...
        return self.tr(
            ...
            "their file size. The output is always a Cloud Optimized "
            "GeoTIFF, never a .jpg file.</p>"
            ...
            "them quickly.</p>"
            "<p>Without pyramids, QGIS reads every pixel in the file to "
            "draw a zoomed-out view, and does it again on every pan and "
            "every zoom.</p>"
            "<p>Without tiling, QGIS cannot read one corner of the "
            "image without reading the full width of every row that "
            "corner sits in.</p>"
            "<p>This tool adds both, and picks a compression setting to "
            "match what the file contains.</p>"

            "<p>What will you use this file for</p>"
            "<p><i>Analysis</i> keeps every pixel value exactly as it "
            "is. Use it for anything you take numbers from: vegetation "
            "indices, crown segmentation, classification, change "
            "detection.</p>"
            "<p><i>Viewing</i> discards detail the eye will not notice, "
            "which makes the file much smaller. A typical drone "
            "orthomosaic came out around 80% smaller in testing. Use it "
            "for basemaps, client copies, QField backdrops and site "
            "context.</p>"
            "<p>Both load and pan at the same speed. The choice affects "
            "file size, and whether pixel values survive unchanged.</p>"
            "<p>Not every file can be written for viewing. Elevation, "
            "16-bit and multispectral imagery are always written for "
            "analysis, and so are some 8-bit RGB files, depending on "
            ...
            "thing, including over a network.</li>"
            "<li>Collar: the transparent border around a survey "
            "area, where the image does not fill the rectangular "
            "file.</li>"
            ...
            "copies of the image at successive zoom levels.</li>"
